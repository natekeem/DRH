...
const base=process.env.DRH_URL||'http://127.0.0.1:5174',out='artifacts/brand-v2'
...
try{
 for(const [width,height]of [[1920,1080],[1440,1000],[390,844]]){
  ...
  for(const slug of canaries){
   const card=page.locator('.reference-card').filter({has:page.locator('a[href="/reference/admd-'+slug+'"]')});await card.scrollIntoViewIfNeeded();const el=card.locator('.bc-card');await el.waitFor()
  ...
  await sharp({create:{width:1280,height:690,channels:3,background:'#eee'}}).composite(thumbs.map((input,i)=>({input,left:(i%4)*320,top:Math.floor(i/4)*230}))).png().toFile(out+'/cards-'+width+'.png')
  for(const slug of canaries){
   ...
   if(width===1440||width===390){await expanded.getByRole('combobox',{name:'카탈로그 섹션 이동'}).selectOption('buttons');await page.screenshot({path:out+'/components-'+slug+'-'+width+'.png'})}
   ...
   if(width===1440){await expanded.getByRole('combobox',{name:'카탈로그 섹션 이동'}).selectOption('geometry');await page.screenshot({path:out+'/geometry-'+slug+'-'+width+'.png'})}
   await expanded.getByRole('combobox',{name:'카탈로그 섹션 이동'}).selectOption('source');assert(await expanded.evaluate(e=>e.scrollTop)>0)
...
console.log('PASS: 36 Card/Detail/Expanded cases, keyboard, touch, source coverage, offline, no external requests')
