...
export function ReferencePage(){
  ...
  return <main className="reference-workspace-page">
    ...
    <div className="reference-workspace">
      <section className="workspace-preview" aria-label="Live preview"><div className="preview-meta"><span>{item.category} · {categoryKo[item.category]} / {item.subcategory}</span><span>01 / SEE</span></div><div className="workspace-demo-shell"><div className="demo-toolbar"><span><i/>{isVendor?'BRAND DESIGN PREVIEW':item.designSystem?'DESIGN SYSTEM PREVIEW':maturity.kind==='external'?'SOURCE REFERENCE':'LIVE PREVIEW'}</span>{maturity.kind!=='external'&&<button onClick={()=>setReplay(v=>v+1)}><RotateCcw size={13}/> Replay</button>}</div><div className={`detail-demo workspace-demo ${item.category==='Pages'||item.category==='Background'?'wide-preview':''}`} key={item.id+replay}>{item.designSystem?<DesignSystemPreview tokens={item.designSystem}/>:maturity.kind==='external'?<div className="source-only-preview"><h2>원본에서 확인하는 Reference</h2><p>이 항목은 코드와 이미지를 재배포하지 않습니다.</p><a href={item.source.url} target="_blank" rel="noreferrer">Source 열기 ↗</a></div>:<DemoRenderer kind={item.demo} detail preset={preset}/>}</div></div><div className="preview-description"><h2>{descriptionKo(item)}</h2><p>{isVendor?'VoltAgent awesome-design-md (MIT)에서 가져온 토큰 기반 오프라인 미리보기입니다. 외부 네트워크 요청 없음.':item.designSystem?'문서에 정의된 토큰의 시각화입니다. 외부 브랜드 화면을 복제하지 않습니다.':maturity.description}</p>{item.demo==='shader-gradient'&&<p>공식 프리셋 · {preset.title}</p>}{isVendor&&vendorMdLoading&&<p style={{opacity:0.5,fontSize:'0.8em'}}>DESIGN.md 로딩 중…</p>}<div className="tag-list">{item.tags.filter(t=>!['vendor','awesome-design-md'].includes(t)).slice(0,6).map(t=><span key={t}>#{t}</span>)}</div></div></section>
      <ArtifactWorkspace key={item.id+(vendorRawMd?'loaded':'pending')} artifacts={artifacts}/>
    </div>
    <section className="workspace-context"><div><span className="eyebrow">WHERE IT FITS</span><h2>이런 화면에 사용하세요.</h2><div className="tag-list">{item.useCases.map(x=><span key={x}>{x}</span>)}</div><p>{item.implementation.framework} · {item.implementation.dependencies.join(', ')||'추가 의존성 없음'}</p></div>{item.source.name !== 'Design Reference Hub' && <aside><span className="eyebrow">SOURCE / LICENSE</span><h2>{item.source.name}</h2><p>{provenanceFor(item)}</p><p>{item.license.notes}</p><div className="workspace-source-links">{item.source.url && <a href={item.source.url} target="_blank" rel="noreferrer">원본 출처 <ExternalLink size={14}/></a>}{item.source.repository&&<a href={item.source.repository} target="_blank" rel="noreferrer">Repository ↗</a>}{item.license.evidenceUrl&&<a href={item.license.evidenceUrl} target="_blank" rel="noreferrer">License evidence ↗</a>}</div></aside>}</section>
