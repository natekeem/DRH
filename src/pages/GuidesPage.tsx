...
export function GuideArticle(){
  ...
  return <main className="guide-article"><Link className="text-link" to="/guides">← 가이드 목록</Link><header><span className="eyebrow">{guide.category} · {guide.readingTime}</span><h1>{guide.title}</h1><p>{guide.description}</p><small>Updated {guide.updatedAt}</small></header>{!content||content.slug!==slug?<p role="status">가이드를 불러오는 중입니다…</p>:content.error?<p role="alert">가이드를 불러오지 못했습니다. 페이지를 새로고침해 주세요.</p>:<GuideMarkdown text={content.text}/>}<aside className="guide-sources"><h2>Source attribution</h2>{guide.sources.map(s=><a key={s.url} href={s.url} target="_blank" rel="noreferrer">{s.label} ↗</a>)}</aside><Link className="text-link" to="/explore?category=DESIGN.md">DESIGN.md 둘러보기 →</Link></main>
}
