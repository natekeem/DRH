...
export function ArtifactWorkspace({artifacts}:{artifacts:ArtifactView[]}) {
  ...
  return <section className="artifact-workspace" aria-label="Artifact workspace">
    <div className="artifact-heading"><span className="eyebrow">TAKE IT TO YOUR AGENT</span><Link to="/guides/using-design-md">사용 가이드 ↗</Link></div>
    ...
    <div id="artifact-panel" className="artifact-panel" role="tabpanel" aria-labelledby={`artifact-tab-${current.id}`} tabIndex={0}><div className="artifact-file"><span>{current.filename}</span><span>{text.length.toLocaleString()} chars</span></div><pre key={current.id+mode}>{text}</pre></div>
