...
export function VendorDesignPreview({entry,detail=false}:{entry:VendorEntry;detail?:boolean}){
 ...
 const jump=<select className="bc-control bc-jump" aria-label="카탈로그 섹션 이동" value="" onChange={e=>{const target=e.currentTarget.closest('.bc-root')?.querySelector('[data-section="'+e.target.value+'"]');target?.scrollIntoView({block:'start'})}}><option value="">섹션 이동</option>{catalog.sections.map(s=><option value={s.id} key={s.id}>{s.label}{s.count?' · '+s.count:''}</option>)}</select>
 ...
 return <div className="bc-root bc-detail" style={catalogStyle(theme)} data-brand={entry.slug} data-theme={theme.id} data-full-spec={ready}>
  <header className="bc-header">{identifier}<div className="bc-header-actions">{controls}{jump}<button ref={trigger} className="bc-control" onClick={()=>setExpanded(true)} disabled={!ready}>크게 보기 ↗</button></div></header>
