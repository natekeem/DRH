...
export function VendorDesignPreview({entry,detail=false}:{entry:VendorEntry;detail?:boolean}){
 ...
 const identifier=<span className="bc-identifier" data-asset-format={asset.format} style={{fontFamily:resolveBrandFont(catalog.roles[0]?.[1]||{}).stack}}>{asset.type!=='text-wordmark'&&<img src={import.meta.env.BASE_URL+(theme.id==='dark'&&asset.darkSrc?asset.darkSrc:asset.src)} alt=""/>}<b>{asset.label}</b><small>{entry.category}</small></span>
 ...
 return <div className="bc-root bc-detail" style={catalogStyle(theme)} data-brand={entry.slug} data-theme={theme.id} data-full-spec={ready}>
  <header className="bc-header">{identifier}<div className="bc-header-actions">{controls}<JumpMenu sections={catalog.sections.filter(s=>s.kind!=='other')} /><button ref={trigger} className="bc-control" onClick={()=>setExpanded(true)} disabled={!ready}>크게 보기 ↗</button></div></header>
