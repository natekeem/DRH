...
export function VendorDesignPreview({entry,detail=false}:{entry:VendorEntry;detail?:boolean}){
 ...
 return <div className="bc-root bc-detail" style={catalogStyle(theme)} data-brand={entry.slug} data-theme={theme.id} data-full-spec={ready}>
  <header className="bc-header">{identifier}<div className="bc-header-actions">{controls}<JumpMenu sections={experienceSections} /><button ref={trigger} className="bc-control" onClick={()=>setExpanded(true)} disabled={!ready}>크게 보기 ↗</button></div></header>
