...
export function BrandComponentSample({component,catalog,theme}:{component:CatalogComponent;catalog:BrandCatalog;theme:BrandTheme}){
 ...
 else if(kind==='navigation')sample=<nav style={{...style,height:undefined,minHeight:style.height}} aria-label={label}>{['Home','Library','Account'].map((v,i)=><button key={v} aria-current={navIndex===i?'page':undefined} onClick={()=>setNavIndex(i)}>{v}</button>)}</nav>
