...
export function CopyButton({text,label='복사'}:{text:string;label?:string}){
 ...
 return <><button onClick={copy}>{state==='copied'?<Check size={14}/>:<Copy size={14}/>} {state==='copied'?'복사됨':label}</button><span role="status" className="copy-error">{state==='error'?'복사하지 못했습니다. 아래 텍스트를 선택하거나 다운로드하세요.':''}</span></>
}
