...
export function resolveArtifacts(item: ReferenceItem, preset?: OfficialShaderPreset): ReferenceArtifacts {
  ...
  if(item.demo?.startsWith('vendor-design-md:')){
    ...
    const specimenContract = entry ? "## Brand Preview Implementation Contract\n\n- Goal: show source DESIGN.md differences through a DRH composition, not an official site replica.\n- Canonical renderer: src/components/demos/VendorDesignPreview.tsx and vendorDesignPreview.css.\n- Card and Detail use the same Browse BrandDesignSpec below. Full source evidence is loaded only for Detail.\n- Render the selected display fallback, weight, tracking and relative size; source primary/surface geometry, palette, spacing and radius.\n- Detail adds the full typography hierarchy, palette, scales and component disclosures. Primary toggles pressed state; Secondary resets it. Marketplace search is local text input only.\n- Labels, figures and image placeholders are DRH examples. Photography and proprietary fonts are not included.\n- Use fluid layouts and bounded Card typography. No desktop-page scaling, animation, shaders or external media.\n- Unmount offscreen Cards and abort Detail fetch on navigation. Preserve keyboard interaction and touch targets.\n- MIT analysis provenance does not license proprietary logos or fonts. Asset rights are separate.\n- Acceptance: responsive Card/Detail, masked comparison, no overflow, reduced motion, offline/local interaction, audit:brand-designs.\n\n" + "Composition: " + entry.spec.layout + "\nFull spec: /brand-design-specs/" + slug + ".json\n\n```json\n" + JSON.stringify(entry.spec,null,2) + "\n```\n" : ""
    // Tokens artifact
    const tokensJson=t?JSON.stringify({format:'drh-design-tokens-v1',source:'VoltAgent/awesome-design-md',slug,upstreamCommit:UPSTREAM_COMMIT,...t,brandSpec:entry?.spec,allColors:entry?.colors,provenance:dProv},null,2):'{}'
    return {
