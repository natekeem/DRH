import { brandIdentifier, brandFontAssets } from './brandPresentation'
import brandOfficialResources from '../data/brandOfficialResources.json'
...
const vendorProvenance = (slug:string):ArtifactProvenance => {
 const entry=vendorEntryBySlug[slug],repo=entry?.upstreamRepo||UPSTREAM_REPO,commit=entry?.upstreamCommit||UPSTREAM_COMMIT
 return {origin:'upstream-oss',sourceUrl:repo+'/blob/'+commit+'/'+entry.upstreamPath,repository:repo,license:'MIT',evidenceUrl:repo+'/blob/'+commit+'/LICENSE',notices:(entry.referenceRights||'')+' '+(entry.copyright||'Copyright (c) 2026 VoltAgent')+'; MIT repository documentation. Preserve the full upstream LICENSE. Brand trademarks, logos, fonts and third-party assets have separate rights.',reviewedAt:entry.importedAt,sourceRevision:commit}
}
const vendorDerivedProvenance=(slug:string):ArtifactProvenance=>({...vendorProvenance(slug),origin:'derived'})
...
export function resolveArtifacts(item: ReferenceItem, preset?: OfficialShaderPreset): ReferenceArtifacts {
  ...
  if(item.demo?.startsWith('vendor-design-md:')){
    ...
    const UPSTREAM_REPO=entry?.upstreamRepo || 'https://github.com/VoltAgent/awesome-design-md'
    const UPSTREAM_COMMIT=entry.upstreamCommit
    const sourceName=entry.sourceName || 'VoltAgent awesome-design-md'
    const copyright=entry.copyright || 'Copyright 2026 VoltAgent'
    const vProv=vendorProvenance(slug)
    const dProv=vendorDerivedProvenance(slug)
    const t=entry?.tokens
    const upstreamPath=`/vendor/${entry.vendorSource || 'awesome-design-md'}/${encodeURIComponent(slug)}/DESIGN.md`
    ...
    const compact=`# ${item.name} — Agent Design Contract\n\nDesign analysis via ${sourceName}; not an official brand design system.\n\n## Provenance\nSource: ${UPSTREAM_REPO}/blob/${UPSTREAM_COMMIT}/${entry.upstreamPath}\nCommit: ${UPSTREAM_COMMIT}\nLicense: MIT repository documentation (${copyright}). ${entry.referenceRights||''} Preserve the full upstream notice. Brand trademarks, logos, fonts and images have separate rights.\n\n## Visual target\n${item.description}\n\n## Use\nRead the unmodified DESIGN.md and full JSON; implement the shared catalog contract below. Keep declared and rendered fonts separate, retain asset restrictions, and validate responsive, keyboard, touch and offline behavior.\n`
    // Extended agent artifact
    const extended=`# ${item.name} — Agent Design Package\n\n> Design analysis derived from publicly visible ${item.name} web design, distributed through ${sourceName}. This is NOT an official ${item.name} design system.\n\n## Source\n- Repository: ${UPSTREAM_REPO}\n- Path: design-md/${slug}/DESIGN.md\n- Upstream commit: ${UPSTREAM_COMMIT}\n- License: MIT (${copyright}; repository content only)\n- Reference rights: ${entry.referenceRights||'Brand asset rights remain separate'}\n- Imported by Design Reference Hub: ${entry.importedAt}\n\n## Description\n${item.description}\n\n## Normalised DRH Tokens\n${t?`\`\`\`json\n${JSON.stringify(t,null,2)}\n\`\`\``:'Tokens not available.'}\n\n## Upstream Colors\n${entry?Object.entries(entry.colors).map(([k,v])=>`- \`${k}\`: ${v}`).join('\n'):'N/A'}\n\n## Implementation Guide\n\n### Step 1: Download DESIGN.md\nCopy the full raw DESIGN.md from the DESIGN.md tab in DRH, or download it directly.\nThis file is the upstream snapshot and is the authoritative design reference.\n\n### Step 2: Point your agent to it\nPlace DESIGN.md in your project root. Explicitly tell your coding agent to read ./DESIGN.md; do not assume automatic discovery.\n\n### Step 3: Use the tokens\nApply the normalised tokens above as CSS custom properties or Tailwind theme variables.\nFor any proprietary font families, use the system-ui fallbacks already provided.\n\n## Do\n- Follow the visual rules in the raw DESIGN.md.\n- Use system font fallbacks for proprietary fonts.\n- Reference this document's MIT license when distributing.\n\n## Don't\n- Claim this as an official ${item.name} design resource.\n- Download proprietary brand fonts or logos.\n- Use this to reproduce a brand's exact copyrighted UI.\n\n## License\nUpstream DESIGN.md: MIT — ${sourceName}\nDRH preview and derived artifacts: MIT — natekeem/DRH\nUpstream commit: ${UPSTREAM_COMMIT}\n`
    const specimenContract = entry ? "## Brand Experience V2.4 Implementation Contract\n\n- Canonical renderer: src/components/demos/VendorDesignPreview.tsx, BrandCatalog.tsx, BrandExperienceCatalog.tsx, BrandAppliedPreview.tsx, BrandComponentSample.tsx, brandExperience.css and vendorDesignPreview.css.\n- Card uses the compact BrandDesignSpec below for identity, evidence-based DNA, micro components and palette.\n- Detail and Expanded MUST load the full spec JSON before rendering the shared catalog. The compact projection is not the full component inventory.\n- Normalize the full spec with src/lib/brandCatalog.ts into BrandCatalogSection arrays; preserve every source component and typography role.\n- Use the closed src/lib/brandComponentStyle.ts normalizer. Preserve bg/fg, border edges, font shorthand, nested typography and declared metrics. Preview projection caps display size and padding without editing source. Separate neutral catalog chrome from brand specimen styles.\n- Component Library and Applied Canvas share BrandDesignSpec, buildBrandCatalog and the closed style normalizer. src/lib/brandExperience.ts defines semantic groups and brand/category scenes. Canvas layout, abstract shapes, example content and interactions are DRH compositions, not source components; apply existing source component styles when available. No invented branded assets or state styles.\n- Continue in your project uses src/lib/referenceHandoff.ts over resolveArtifacts; keep source path/revision/hash, resource registry and compact/extended Agent parity.\n- Render semantic hero, section, CTA, pricing, media, icon shape, legal and footer specimens as well as actual button, input, tab, card, badge, dialog, table and navigation specimens only where source definitions exist; source properties remain inspectable.\n- Theme switching is available only with explicit complementary canvas and ink roles. Keep all sections in the selected theme.\n- Official links are a separately verified registry in src/data/brandOfficialResources.json, not analysis provenance or an asset license. The separate brandVerifiedAssets.json overlay supplies verified local logos; preserve each usage restriction and attribution. Never treat a logo as an MIT asset.\n- Expanded uses the same catalog model and the full technical renderer, a native modal, focus restoration, Escape, body scroll lock and responsive single-column mobile layout.\n- Preserve original source typography sizes as annotations and cap samples to the container. Detail order: Overview, Design DNA, Component Library, Applied Canvas, Resources. Semantic palette and Display/Heading/Body/Caption roles disclose original values and missing declarations; Expanded retains all definitions. Shared ReferenceProjectHandoff sits at the bottom of every reference page. Elevation, motion and source-backed responsive rules have independent sections. Raw evidence is disclosed explicitly. Use src/lib/brandPresentation.ts and src/data/brandFontAssets.json for declared versus rendered font resolution. Load local OFL font files with their license notices; OS fonts use system stacks; proprietary fonts are never bundled. No remote fonts, hotlinked images or website HTML.\n- Labels and data are local DRH examples, not official brand UI. MIT analysis does not license brand assets.\n- Validate responsive Card/Detail/Expanded, theme evidence, keyboard and touch interaction, reduced motion, offline behavior and audit:brand-designs.\n\nFull spec: /brand-design-specs/" + slug + ".json\nLayout DNA: " + entry.spec.layout + "\n\nCompact Browse projection (load full spec for the catalog):\n\n```json\n" + JSON.stringify(entry.spec,null,2) + "\n```\n\nOfficial resources (separate from provenance):\n" + JSON.stringify((brandOfficialResources as Record<string,unknown[]>)[slug]||[],null,2) + "\nFont registry: src/data/brandFontAssets.json\nVerified asset registry: src/data/brandVerifiedAssets.json\nPresentation rules: src/lib/brandPresentation.ts\nKorean summaries: src/data/brandCuratedCopy.json (use only on an exact source-row match)\n\nVerified brand identifier (usage restrictions apply):\n" + JSON.stringify(brandIdentifier(slug,entry.spec),null,2) + "\n\nLocal font registry (preserve each license notice):\n" + JSON.stringify(brandFontAssets,null,2) + "\n" : ""
    const officialResources = (brandOfficialResources as Record<string,unknown[]>)[slug] || []
    const tokensJson=t?JSON.stringify({format:'drh-design-tokens-v1',source:sourceName,slug,upstreamCommit:UPSTREAM_COMMIT,...t,brandSpec:entry?.spec,fullSpecPath:'/brand-design-specs/'+slug+'.json',officialResources,fontAssets:brandFontAssets,brandAsset:entry?brandIdentifier(slug,entry.spec):undefined,allColors:entry?.colors,provenance:dProv},null,2):'{}'
    return {
      ...
      agent:{extended:extended+'\n\n'+specimenContract,compact:compact+'\n\n'+specimenContract,filename:`${item.id}-agent-package.md`,provenance:dProv},
    }
  }
