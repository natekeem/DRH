...
export function CopyBlock({ label, text, code=false }:{label:string;text:string;code?:boolean}) {
  ...
  return <section className="copy-block"><header><span>{label}</span><button onClick={copy}>{copied?<><Check size={14}/> 복사됨</>:<><Copy size={14}/> 복사</>}</button></header><pre className={code?'code':''}>{text}</pre></section>
}
